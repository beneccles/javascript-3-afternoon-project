...
let evenNumbers = mixedNumbers.filter(function(element, index, wholeArray){
  ...
  // Filter brings in the array through a callback, and then compares each item in the array. If the comparsion is true, return true; Otherwise return false. 
  // Filter will then take care of adding the value to the new array to return.
})
...
let orderTotals = orders.map(function(el, i, arr){
  let taxVal = arr[i].tax + 1;

  return arr[i].price * taxVal;
});
